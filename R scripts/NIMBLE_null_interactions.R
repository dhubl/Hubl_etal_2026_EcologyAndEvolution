...
##this model has no interactions between the species 
...
MS.none <- nimbleCode({  
  ...
  for (j in 1:nsites[1]) { #first season
    ...
    f12[1,j] <- 0 # skunk x coyote interaction
    f13[1,j] <- 0 # skunk x bobcat interaction
    f23[1,j] <- 0 # coyote x bobcat interaction
  ...
  for (j in 1:nsites[2]) { #season 2 with autoregression function
    ...
    f12[2,j] <- 0 # skunk x coyote interaction
    f13[2,j] <- 0 # skunk x bobcat interaction
    f23[2,j] <- 0 # coyote x bobcat interaction
  ...
  #Priors  
  #using shrinkage priors for all except intercepts and autoregressive parameters
...
#inits
z.int <- matrix(NA, nrow = 2, ncol = 712) #set initial value for latent state. find highest possible community state for each site
...
saveRDS(samples.none,"null_Interactions.rds")

##check diagnostics
MCMCtrace(object = samples.none$samples,
          pdf = FALSE,
          ind = TRUE,
          params= c('ps0','ps.camera','ps.moon',
                    'pc0','pc.camera','pc.moon',
...
tmp <- grep("z",colnames(samples.none$samples$chain1)) #ignore the latent state estimates
samples.none$summary$all.chains[-tmp,]
#Gelman-Rubin diagnostic
gelman.diag(samples.none$samples[,-tmp])
