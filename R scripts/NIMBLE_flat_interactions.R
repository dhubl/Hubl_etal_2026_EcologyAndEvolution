...
##this model has constant interaction effects (just the intercept modeled in second order parameters)
...
#2026
############
### Intercept Interactions only ##################################################################################

MS.intercept <- nimbleCode({  
  #Likelihood   
  #t=time/rep
  #j=site
  #k=survey replicate
  ...
  for (j in 1:nsites[1]) { #first season
    ...
    f12[1,j] <- sc0 # skunk x coyote interaction
    f13[1,j] <- sb0 # skunk x bobcat interaction
    f23[1,j] <- cb0 # coyote x bobcat interaction
  ...
  for (j in 1:nsites[2]) { #season 2 with autoregression function
    ...
    f12[2,j] <- sc0 # skunk x coyote interaction
    f13[2,j] <- sb0 # skunk x bobcat interaction
    f23[2,j] <- cb0 # coyote x bobcat interaction
    
  }#j
  
  #observation model
  # 
  #CH.effort[] is a matirx that indicates if a survey was conducted or not, 1 if yes, 0 if no
  for (t in 1:rep){
    for(j in 1:nsites[t]){
      for(k in 1:n.occ){
  ...
  #Priors  
  #using shrinkage priors on all except intercepts and autoregressive parameter
...
saveRDS(samples.intercept,"flat_Interactions.rds") #save model results for future use

##check diagnostics             
MCMCtrace(object = samples.intercept$samples,
          pdf = FALSE,
          ind = TRUE,
          params= c('ps0','ps.camera','ps.moon',
                    'pc0','pc.camera','pc.moon',
...
tmp <- grep("z",colnames(samples.intercept$samples$chain1)) #find position of latent state estimates
#summary 
samples.intercept$WAIC
samples.intercept$summary$all.chains[-tmp,]


#Gelman-Rubin diagnostic
gelman.diag(samples.intercept$samples[,-tmp])
